...
/* funções auxiliares */
int converteParacm(Altura *p) { return p->m * 100 + p->cm; }
/* Funções exportadas */
...
void alt_libera(Altura *p) { free(p); }
...
void alt_exibe(Altura *p) { printf("\n %d,%02dm", p->m, p->cm); }
...
Altura *alt_soma(Altura *p, int cm) {
  int total_cm = converteParacm(p) + cm;
  int novos_metros = total_cm / 100;
  int novos_centimetros = total_cm % 100;
  return alt_cria(novos_metros, novos_centimetros);
}
